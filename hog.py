...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    turn_score, current_num_rolls = 0, 1
    pig_out = 0
    while current_num_rolls <= num_rolls:
        roll = dice()
        if roll == 1:
            pig_out = 1
        else:
...
def both(f, g):
    ...
    def comment(score0, score1):
        f_updated, g_updated = f(score0, score1), g(score0, score1)
        return both(f_updated, g_updated)
    return comment
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    averaged_turn = make_averaged(roll_dice, num_samples) 
    highest_average_turn_score, num_rolls = 0, 1
    while num_rolls <= 10:
        average_turn_score = averaged_turn(num_rolls, dice)
        if average_turn_score > highest_average_turn_score:
            highest_average_turn_score, optimal_rolls = average_turn_score, num_rolls
        num_rolls += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    free_bacon_score = score + free_bacon(opponent_score) # turn score after free bacon
    if score < opponent_score and is_swap(free_bacon_score, opponent_score):
        return 0
    elif free_bacon(opponent_score) >= margin and not(is_swap(free_bacon_score, opponent_score)):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
